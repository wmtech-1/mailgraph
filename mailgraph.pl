...
my $year;
my $this_minute;
my %sum = ( sent => 0, received => 0, bounced => 0, rejected => 0, spfnone => 0, spffail => 0, spfpass => 0, dmarcnone => 0, dmarcfail => 0, dmarcpass => 0, dkimnone => 0, dki
mfail => 0, dkimpass => 0, virus => 0, spam => 0 );
my $rrd_inited=0;
...
sub process_line($)
{
        ...
        if($prog =~ /^postfix\/(.*)/) {
                ...
                if($prog eq 'smtp') {
                        ...
                        if($text =~ /^(?:[0-9A-Za-z]+: |NOQUEUE: )?reject: /) {
                                event($time, 'rejected');
                        }
                }
                elsif($prog eq 'error') {
...
sub update($)
{
        ...
        print "update $this_minute:$sum{sent}:$sum{received}:$sum{bounced}:$sum{rejected}:$sum{spfnone}:$sum{spffail}:$sum{spfpass}:$sum{dmarcnone}:$sum{dmarcfail}:$sum{dmarcp
ass}:$sum{dkimnone}:$sum{dkimfail}:$sum{dkimpass}:$sum{virus}:$sum{spam}\n" if $opt{verbose};
        RRDs::update $rrd, "$this_minute:$sum{sent}:$sum{received}:$sum{bounced}:$sum{rejected}:$sum{spfnone}:$sum{spffail}:$sum{spfpass}:$sum{dmarcnone}:$sum{dmarcfail}:$sum{
dmarcpass}:$sum{dkimnone}:$sum{dkimfail}:$sum{dkimpass}" unless $opt{'only-virus-rrd'};
